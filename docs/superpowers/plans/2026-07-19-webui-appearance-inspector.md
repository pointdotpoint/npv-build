...
- `hair_color`: appearance names of any `hh_*_{rig}` app, deduplicated, stripped of `^\d+_` is NOT applied (keep raw names like `03_ginger_copper` — that is what the selection raw stores). **AMENDED 2026-07-19 (Task 2 review finding):** the real part index contains no `hh_*` apps at all, so this rule alone yields nothing. Hair colours are global vanilla constants (hair-profile `.hp` stems in `base\characters\common\hair\textures\hair_profiles\`, extracted from the user's install — 71 profiles, of which the `custom_*` NPC-specific ones are excluded). Rule: when the index-derived list is empty AND the index itself is non-empty, fall back to the vendored `npv_build/data/hair_colors.json` (sorted list of non-`custom_` stems, strings only). `option_lists(None/{}) == {}` stays as-is (no index → whole feature degrades).
...
- **AMENDED 2026-07-19 (Task 7 live-gate finding):** deriving the token from the ARCHIVE FILENAME fails on real mods whose archive name diverges from internal content names (e.g. `ANRUI_MiyaviHair_Fluffypony_CCXL.archive` contains `fhair_miyavi_fluffytail.app` — "fluffypony" never appears in any internal path, so all-token matching can never succeed). Fixed contract: after install, `add_hair_mod` lists the installed `.archive` itself for `.app` files (WolvenKit adapter, one listing), picks the best hair `.app` (non-`\fpp\`, basename starting `fhair_`/`mhair_` preferred by rig gender, non-cyb/shaved preferred), derives the token from that `.app` basename (strip the `fhair_`/`mhair_` prefix), then validates that `extract_hair_components(game_dir, token, rig)` resolves — guaranteeing build-time resolution uses a token that provably works. No hair `.app` in the archive → structured error, unchanged.
