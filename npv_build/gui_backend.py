    ...
    def _run(self, **kwargs):
        ...
        def on_event(ev):
            ...
            if ev.kind == "stage_started":
                ...
                self.queue.put(
                    ("stage", {"stage": ev.stage, "status": "started", "message": ev.message})
                )
                ...
                self.queue.put(
                    ("stage", {"stage": ev.stage, "status": status, "message": ev.message})
                )
                if ev.stage in stages:
                ...
                self.queue.put(
                    ("stage", {"stage": ev.stage, "status": "failed", "message": ev.message})
                )
...
def resolve_tool_paths() -> dict:
    ...
    if not wolvenkit:
        for candidate in (
            tools_dir / "wolvenkit" / f"WolvenKit.CLI{ext}",
...
def summarize_cc(cc_settings: dict) -> dict:
    ...
    return {
        ...
        "hair_color": (
            hair.get("color") or hair_color_from_selections(selection_rows) or "Unknown"
        ),
