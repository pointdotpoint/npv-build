...
def test_stock_head_morph_fallback_targets_generated_component_name():
    """A failed Blender bake must still preserve V's face via the stock morph.

    The actual base-head component name is generated by the game, so the
    fallback must not rely on an ``h0_`` prefix that does not exist in real
    extracted data.
    """
    from npv_build.blender_module import HEAD_MORPHTARGET
    from npv_build.wolvenkit import _enable_stock_head_morph_fallback

    stock_mt = HEAD_MORPHTARGET["pwa"]
    components = [
        {
            "comp_type": "entSkinnedMeshComponent",
            "name": "MorphTargetSkinnedMesh7243",
            "mesh": "base\\characters\\head\\player_base_heads\\player_female_average\\head.mesh",
            "morph_resource": stock_mt,
        },
        {
            "comp_type": "entSkinnedMeshComponent",
            "name": "MorphTargetSkinnedMesh7561",
            "mesh": "base\\characters\\head\\player_base_heads\\player_female_average\\personal_link.mesh",
        },
    ]

    assert _enable_stock_head_morph_fallback(components, "pwa") == "MorphTargetSkinnedMesh7243"
    head = components[0]
    assert head["comp_type"] == "entMorphTargetSkinnedMeshComponent"
    assert head["morph_resource"] == stock_mt
    assert head["graph"] == stock_mt
    assert head["source"] == "stock morph target head (programmatic fallback)"
    assert components[1]["comp_type"] == "entSkinnedMeshComponent"


def test_stock_head_morph_fallback_refuses_default_face():
    """Missing extracted morph data must fail instead of yielding default V."""
    from npv_build.wolvenkit import _enable_stock_head_morph_fallback

    with pytest.raises(WolvenKitError, match="default-face NPV"):
        _enable_stock_head_morph_fallback(
            [{"name": "MorphTargetSkinnedMesh7243", "mesh": "base\\x\\head.mesh"}],
            "pwa",
        )
