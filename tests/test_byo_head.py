...
from npv_build.head_bake import prepare_head, _finalize_head, _get_glb_vertex_count

def test_cli_no_restore_without_head_mesh():
    # If --no-restore-head-materials is passed without --head-mesh
    with patch("sys.argv", ["npv-build", "my_v", "--output", "out_dir", "--no-restore-head-materials"]):
...
def test_cli_both_head_flags():
    ...
    with patch("sys.argv", ["npv-build", "my_v", "--output", "out_dir", "--head-glb", "foo.glb", "--head-mesh", "bar.mesh"]):
        with pytest.raises(SystemExit):
            main()

def test_cli_missing_head_glb():
    with patch("sys.argv", ["npv-build", "my_v", "--output", "out_dir", "--head-glb", "nonexistent.glb"]):
        with pytest.raises(SystemExit):
            main()
...
def test_cli_wrong_extension_head_glb(tmp_path):
    ...
    with patch("sys.argv", ["npv-build", "my_v", "--output", "out_dir", "--head-glb", str(wrong_file)]):
        with pytest.raises(SystemExit):
            main()
...
def test_cli_valid_glb_override(mock_load_config, mock_run_orchestrator, tmp_path):
    ...
    with patch("sys.argv", ["npv-build", "my_v", "--output", "out_dir", "--cc-json", str(cc_json), "--head-glb", str(valid_glb)]):
        main()
...
def test_heb_dropped_with_override_no_heb_mesh(mock_inject, mock_find_stock, mock_prepare_head, tmp_path):
    ...
    with patch("npv_build.wolvenkit._extract_part_components") as mock_extract:
        mock_extract.return_value = [
            {"name": "h0_000_pwa_c__basehead", "mesh": "stock_head.mesh", "comp_type": "entSkinnedMeshComponent", "appearance": "default"},
            {"name": "heb_000_pwa__basehead", "mesh": "stock_heb.mesh", "comp_type": "entSkinnedMeshComponent", "appearance": "default"},
        ]
...
@patch("npv_build.wolvenkit.find_stock_head_part")
@patch("npv_build.wolvenkit._inject_components")
def test_heb_repointed_with_override_and_heb_mesh(mock_inject, mock_find_stock, mock_prepare_head, tmp_path):
